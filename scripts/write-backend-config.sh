...
POST_EKS_DIR="$ROOT_DIR/terraform/post-eks"

mkdir -p "$NETWORK_DIR" "$EKS_DIR" "$GITHUB_OIDC_DIR" "$POST_EKS_DIR"
...
cat > "$POST_EKS_DIR/backend.hcl" <<EOF
bucket         = "${BUCKET_NAME}"
...
echo " - terraform/post-eks/backend.hcl"
